...
class FaceRecognitionService {
  ...
  static const int embeddingDim = 192; // 192-d feature vector
  static const double recognitionThreshold = 0.75; // Cosine similarity threshold for a match
  ...
  Future<List<double>> extractFaceEmbedding(img.Image croppedFace) async {
    // 1. Resize to MobileFaceNet input size (112x112)
    ...
    if (_isModelLoaded && _interpreter != null) {
      try {
        ...
        // Output tensor shape: [1, embeddingDim]
    ...
    // 2. Fallback feature extractor (Perceptual Block Grid Moments)
    return _generateFallbackEmbedding(resizedFace);
  }
  ...
  /// Combines and averages multiple embedding vectors (from multi-angle enrollment) into a single master vector
  ...
  /// Fallback mathematical feature vector for testing when TFLite asset is being downloaded
  List<double> _generateFallbackEmbedding(img.Image image) {
    final List<double> vector = List.filled(embeddingDim, 0.0);
    ...
    for (int cy = 0; cy < 8 && idx < embeddingDim - 64; cy++) {
      for (int cx = 0; cx < 8 && idx < embeddingDim - 64; cx++) {
        double cellSum = 0.0;
        int count = 0;
    ...
    while (idx < embeddingDim) {
      vector[idx] = (math.sin(idx.toDouble()) + 1.0) / 2.0;
      idx++;
    }
